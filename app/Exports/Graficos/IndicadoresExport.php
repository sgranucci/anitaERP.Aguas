        ...
        for ($i = $n - 1; $i < count($this->datas); $i++)
        {
            ...
            case -1:
                ...
                if ($low <= $minimo && $trend == $this->datas[$i-1]['tendencia'])
                {
                    ...
                    $minimo = $low;
                    $this->datas[$i]['provMin'] = $minimo;
                    $this->datas[$i]['barras'] = ABS($bnMin - $bnMax) + 2;
                    $this->datas[$i]['min'] = $minimo;
                    $this->datas[$i]['tendencia'] = $trend;
                    $barras++;
                    $this->datas[$i]['trendBars'] = $barras;
    ...
    private function calculaSwingTgt()
    {
        ...
        for ($i = $n - 1; $i < count($this->datas); $i++)
        {
            ...
            if ($flAbrePosicion)
            {
                ...
                {
                    $this->datas[$i]['p'] = '0';
                    $this->datas[$i]['evento'] = 'Cierre por Tgt Hit';
                ...
                {
                    $this->datas[$i]['p'] = '0';
                    $this->datas[$i]['evento'] = 'Cierre por SL';
                    $flAbrePosicion = false;
                }
            ...
            if ($sp != '')
            {
                ...
                case 'DB':
                    ...
                    // Si llego a otro minimo sin encontrar una entrada inicia un nuevo swing
                    if ($iniciaSwing && $off1oA != -1 && !$flBuscaEntrada && !$flAbrePosicion)
                        $iniciaSwing = false;
                ...
                case 'DT':
                    ...
                    if ($iniciaSwing && !$flAbrePosicion)
                    {
                        ...
                        $vela1oA = $this->datas[$i]['high'] - $this->datas[$i]['low'];
                        $maximo1oA = $this->datas[$i]['high'];
                        $this->datas[$i]['entrada'] = ' Maximo Vela 1oA '.$vela1oA.' Max. 1oA '.$maximo1oA.' Barras Alc. '.$barrasAlcistas;
                    }
                    break;
                }
