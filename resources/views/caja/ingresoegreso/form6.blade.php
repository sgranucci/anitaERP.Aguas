    ...
    <div class="card-body">
    	<table class="table" id="archivo-table">
    		...
    		<tbody id="tbody-tabla-archivo">
		 		@if ($data->caja_movimiento_archivos ?? '') 
					@foreach (old('archivos', $data->caja_movimiento_archivos->count() ? $data->caja_movimiento_archivos : ['']) as $archivo)
            			<tr class="item-archivo">
                			<td>
								<input type="file" name="nombrearchivos[]" class="form-control nombrearchivos" 
									onchange='actualizaArchivo(this)'
									data-initial-preview="{{isset($archivo->nombrearchivo) ? asset("storage/archivos/caja_movimientos/$data->id/$archivo->nombrearchivo") : ''}}" \>
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
                                	...
                                	@else
										<img heigh=100px width=100px src="{{ asset("storage/archivos/caja_movimientos/$data->id/$archivo->nombrearchivo") }}" alt="image">
									@endif
                			...
                			<td>
								@if ($archivo->nombrearchivo ?? '')
									<a download="{{$archivo->nombrearchivo}}" href="{{ asset("storage/archivos/caja_movimientos/$data->id/$archivo->nombrearchivo") }}" title='Descargar' /><i class="fa fa-download"></i>
		...
		@include('caja.ingresoegreso.template6')
        <div class="row">
        	<div class="col-md-12">
